...
const STEPS = [
  { id: 'step-1', icon: Clock, title: 'Doctor logs in late', description: 'LocDoc detects the doctor has not checked in at the scheduled start time.', color: 'text-red-400', bg: 'bg-red-500/15', border: 'border-red-500/25', num: '01' },
  { id: 'step-2', icon: RefreshCw, title: 'Live status updates', description: 'The appointment queue status changes to "Delayed" in real time.', color: 'text-amber-400', bg: 'bg-amber-500/15', border: 'border-amber-500/25', num: '02' },
  { id: 'step-3', icon: Bell, title: 'Patients notified instantly', description: 'All booked patients receive an SMS/push notification with the updated wait time.', color: 'text-cyan-400', bg: 'bg-cyan-500/15', border: 'border-cyan-500/25', num: '03' },
  { id: 'step-4', icon: MessageSquare, title: 'Reschedule or wait', description: 'Patients can reschedule to the next available slot or choose to wait with live updates.', color: 'text-blue-400', bg: 'bg-blue-500/15', border: 'border-blue-500/25', num: '04' },
  { id: 'step-5', icon: CheckCircle2, title: 'Zero ghosting, full trust', description: "No more silent no-shows. Patients always know exactly what\'s happening.", color: 'text-emerald-400', bg: 'bg-emerald-500/15', border: 'border-emerald-500/25', num: '05' },
];
    ...
    >
      ...
      <div className="relative max-w-screen-2xl mx-auto px-4 lg:px-8 xl:px-10">
        ...
        <div className="text-center mb-10 reveal-up">
          ...
          <h2 className="tight-headline text-white mb-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 mb-8">
          {STEPS?.map((step, i) => (
            <div
              key={step?.id}
              className="reveal-up glass-card rounded-2xl p-5 flex flex-col gap-3 bento-hover card-shine group"
            ...
            >
              <div className="flex items-start justify-between">
                ...
                <span className="text-3xl font-black text-white/6 leading-none font-tabular">{step?.num}</span>
              </div>
              <div>
